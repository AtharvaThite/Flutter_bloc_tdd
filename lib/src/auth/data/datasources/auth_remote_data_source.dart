...
import 'package:flutter_bloc_tdd/core/utils/constants.dart';
...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<void> forgotPassword(String email) async {
    try {
      ...
      throw ServerException(
        message: e.message ?? 'Something went wrong with Reset Password',
        statusCode: e.code,
      );
    } catch (e, s) {
  ...
  Future<LocalUserModel> signIn({
    ...
    try {
      ...
      throw ServerException(
        message: e.message ?? 'Something went wrong with Authentication',
        statusCode: e.code,
      );
    } on ServerException {
      rethrow;
  ...
  Future<void> signUp({
    ...
    try {
      ...
      throw ServerException(
        message: e.message ?? 'Something went wrong with SignUp',
        statusCode: e.code,
      );
    } catch (e, s) {
      debugPrintStack(stackTrace: s);
  ...
  Future<void> updateUser({
    ...
    try {
      switch (action) {
        ///Case to update email
        case UpdateUserAction.email:
          await _authClient.currentUser
              ?.verifyBeforeUpdateEmail(userData.toString());
          await _updateUserData({'email': userData});

        ///Case to update displayName
        case UpdateUserAction.displayName:
          await _authClient.currentUser?.updateDisplayName(userData.toString());
          await _updateUserData({'fullName': userData});

        ///case to update password
